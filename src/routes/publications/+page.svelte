<script lang="ts">
    ...
    import { companies, isLoading, errorMessage, publications } from '$lib/stores';
...
<div class="container mx-auto p-8 bg-gray-900 text-gray-200 min-h-screen">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full bg-gray-800 border border-gray-700">
                <thead class="bg-gray-700">
                    <tr>
                        <th class="py-3 px-4 text-left">Documento ID</th>
                ...
                <tbody>
                    {#each $publications as publication}
                        <tr class="hover:bg-gray-700 border-t border-gray-600">
                            <td class="py-2 px-4">{publication.id}</td>
                            <td class="py-2 px-4 font-semibold">{publication.filename}</td>
                            <td class="py-2 px-4">{publication.publicationDate}</td>
                            <td class="py-2 px-4">{publication.fileUrl}</td>
                        </tr>
                    {/each}
                </tbody>
            </table>
    ...
    {:else}
        <div class="bg-gray-800 border border-gray-700 p-6 rounded-md">
            <p class="text-lg text-center">No se encontraron documentos en la base de datos.</p>
        </div>
    {/if}
</div>